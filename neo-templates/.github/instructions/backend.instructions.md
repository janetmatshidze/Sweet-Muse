...
* Names of classes, methods, enumerations, public fields, public properties, namespaces, const: `PascalCase`.
* Names of local variables, parameters: `camelCase`.
* Names of private, protected, internal and protected internal fields and properties: `camelCase`. Do not prefix with `_`.
* Naming convention is unaffected by modifiers such as static, readonly, etc.
* For casing, a "word" is anything written without internal spaces, including acronyms. Acronyms may keep their internal uppercase letters, but choose one style per acronym and use it consistently across the project (e.g. `SLA` stays `SLA`, while `HTTP` may be styled as `Http`).
* When an acronym is the first word of a camelCase identifier (locals, parameters, and camelCase fields/properties), lowercase the whole acronym, e.g. `slaVariable`/`httpVariable`. In PascalCase identifiers it may stay fully uppercase, or be treated as a word, e.g. `SLAClass`/`HttpClass`.
* Names of interfaces start with `I`, e.g. `IInterface`.
* Always prefix member access with `this.`.
...
* Namespace `using` declarations go at the top, inside namespaces. `using` import order is alphabetical, apart from `System` imports which always go first.
* Class member ordering:
  * Group class members in the following order:
    * enums, delegates and events.
...
* A single break before opening brace.
* A single line break between closing brace and `else`.
...
* Line continuations are indented 2 spaces.
...
{
  using System;                                     // `using` goes at the top, inside the namespace.
  ...
  public class MyClass                              // Classes are PascalCase.
  {
    ...
    private Results results;                        // Private member variables are camelCase.
    public static int NumTimesCalled = 0;
    public const int Bar = 100;                     // const are PascalCase.
    private int[] someTable = {                     // Container initializers use a 2 space indent.
      2, 3, 4,
    }
    ...
    {
      results = new Results {
        NumNegativeResults = 1,                     // Object initializers use a 2 space indent.
        NumPositiveResults = 1,
      };
    }

    public int CalculateValue(int mulNumber)
    {                                               // A single line break before opening brace.
      int foo = 2, bar = 3;
      ...
      if (!NoCounting)                              // No space after unary operator and space after `if`.
      {
        ...
        else if (resultValue > 0)                   // A single line break between brace and else.
        {
    ...
    // If aligning argument lines with the first argument does not fit, or is difficult to read, wrap all arguments on new lines with a 2 space indent.
...
* Named tuples are fine for passing data internally within a class.
...
* Discouraged:
  ...
  * When users would clearly benefit from knowing the type - e.g. `var items = GetList();`
...
* Consider using [Named Arguments](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/named-and-optional-arguments) to clarify argument meanings at the call site.
...
```csharp
// Better
DecimalNumber product = CalculateProduct(values, precisionDecimals: 7, useCache: false, completionDelegate: null);
```
best:
```csharp
// Good
...
- Entities should never be added directly to the DbContext, e.g. `this.dbContext.Add(entity);`. They should rather be added via the `DbSet`, e.g. `this.dbContext.Entities.Add(entity);`. This will do a type check and also makes it easier to check where a particular `DbSet` has been used.
