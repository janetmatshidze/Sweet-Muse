...
.PARAMETER ExcludedDatabases
  The names of databases to exclude from restore (SourceDbPrefix is added to these names). This should be provided as a comma-separated string.
...
try {
  ...
  if (($null -ne $restoreContext) -and ($null -ne $restoreContext.TargetConfigs)) {
