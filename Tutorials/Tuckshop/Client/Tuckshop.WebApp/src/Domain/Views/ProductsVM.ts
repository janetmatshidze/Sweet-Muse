...
export default class ProductsVM extends Views.ViewModelBase {
    ...
    public async initialise() {
        ...
        const categoriesResponse = await this.taskRunner.waitFor(this.categoriesApiClient.get());
    ...
    public deleteProduct(product: Product) {
        this.taskRunner.run(async () => {
            if (product.productId) {
                await this.productsApiClient.delete(product.productId);
            ...
            this.notifications.addSuccess("Product deleted", " Product deleted successfully", 4);
            this.editingProduct = null;
        }).catch(() => {
            // taskRunner already showed its own toast for the error.
            // Catching here just prevents React's dev overlay from also firing.
        });
    }
    ...
    public async uploadProductImage(file: File) {
        ...
        if (!allowedTypes.includes(file.type)) {
            this.notifications.addDanger("Invalid file", "Please upload a JPG, PNG, WebP, or GIF image.", 5);
            return;
        }
        await this.taskRunner.run(async () => {
            const authResponse = await this.imageKitApiClient.getAuthParams();

            const result = await upload({
                file,
                fileName: file.name,
                folder: "/Sweet-Muse",
                publicKey: authResponse.data.publicKey,
                signature: authResponse.data.signature,
                expire: authResponse.data.expire,
                token: authResponse.data.token
            });

            if (result.url) {
                this.editingProduct!.imageUrl = result.url;
            } else {
                this.notifications.addDanger("Upload failed", "The image was uploaded but no URL was returned.", 4);
            }
        })
    }
