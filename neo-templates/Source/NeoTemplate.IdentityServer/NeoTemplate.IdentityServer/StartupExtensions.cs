namespace NeoTemplate.IdentityServer
{
  using System;
  ...
  using NeoTemplate.IdentityServer.App.Services.Events;
  using NeoTemplate.IdentityServer.App.Services.UserManagement;
  using NeoTemplate.IdentityServer.Filters;
  using NeoTemplate.IdentityServer.Initializers;
  using NeoTemplate.IdentityServer.Models;
  using NeoTemplate.IdentityServer.Models.Migrations.DesignTime;
  using NeoTemplate.IdentityServer.Models.Migrations.Initializers;
  using NeoTemplate.IdentityServer.Models.Options;
  using Serilog;
  ...
  public static class StartupExtensions
  {
    ...
    {
      services.AddScoped<IDbContextProcessor, AuditTrailProcessor<NeoTemplateApplicationUser>>();
      services.AddScoped<IAuditTrailProcessor>(sp => sp.GetRequiredService<AuditTrailProcessor<NeoTemplateApplicationUser>>());

      services.AddScoped<IUsersDbContext<NeoTemplateApplicationUser>>(sp => sp.GetRequiredService<IdentityDbContext>());

      services.AddNeoClientUserResolver<NeoTemplateApplicationUser, NeoTemplateApplicationUserClaimMapper>();

      services.AddScoped<Neo.Identity.IUserStore<NeoTemplateApplicationUser>, UserStore>();
    ...
    public static IServiceCollection AddIdentityServices(this IServiceCollection services, IWebHostEnvironment environment, IConfiguration configuration)
    {
      ...
      services.AddIdentity<NeoTemplateApplicationUser, IdentityRole>(options =>
          {
      ...
      services.AddIdentityProviderSupport<IdentityDbContext, NeoTemplateApplicationUser>();
    ...
    public static IServiceCollection AddIdentityAsyncInitialisers(this IServiceCollection services, IWebHostEnvironment environment, IConfiguration configuration)
    {
      ...
      services.AddSystemUser<IdentityDbContext, NeoTemplateApplicationUser>(
        new Neo.Model.Identity.SystemUser.SystemUserOptions<NeoTemplateApplicationUser>()
        {
          SuppressInitializer = true,
          FindUserByIdPredicate = (Guid id) => appUser => appUser.Id == id.ToString(),
          CreateSystemUserHandler = (Guid id) => NeoTemplateApplicationUser.SystemUser(id),
        });

      services.AddAsyncInitializer<SeedDataAsyncInitializer>();

      services.AddIdentityConfigSyncServices(
        new Neo.IdentityServer.App.OpenIddict.Services.ConfigSyncServiceOptions() { ApiResourceName = "NeoTemplate.Domain" });
