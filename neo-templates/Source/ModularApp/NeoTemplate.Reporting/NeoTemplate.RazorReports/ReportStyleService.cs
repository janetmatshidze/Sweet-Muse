...
namespace NeoTemplate.RazorReports
{
  ...
  public class ReportStyleService
  {
    ...
    {
      string relativePath = $"/_content/NeoTemplate.RazorReports/{pathFromWebRoot}";
    ...
    public HtmlString RenderReportingStyles(bool isScreenDisplay)
    {
      ...
      AppendResource(stringBuilder, assembly, "NeoTemplate.RazorReports.wwwroot.css.reporting.css");

      this.AppendUtils(stringBuilder);

      if (isScreenDisplay)
      {
        AppendResource(stringBuilder, assembly, "NeoTemplate.RazorReports.wwwroot.css.reporting.screen.css");
      }
