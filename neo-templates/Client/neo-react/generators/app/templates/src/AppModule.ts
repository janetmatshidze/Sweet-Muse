...
const AppModule = new AppServices.Module("App", container => {
    ...
    // Config
    container.bind(<%= typesPath %>.Config).to(AppConfig).inSingletonScope();
<%_ if(hasModules) { _%>
...
<%_ } _%>
    ...
    // Services
<%_ if(!hasModules) { _%>
    container.bind(<%= typesPath %>.Services.DataCache).to(AppDataCache).inSingletonScope();
<%_ } _%>
    container.bind(<%= typesPath %>.Services.AppLayout).to(AppLayout).inSingletonScope();
    container.bind(<%= typesPath %>.Services.RouteService).to(RouteService).inSingletonScope();
