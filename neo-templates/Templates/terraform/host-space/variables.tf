...
variable "sql_servers" {
  ...
  type = map(object({
    ...
    logs_disk_enabled = optional(bool, true)
    ...
    stop_start_schedule = optional(object({
      enabled                  = optional(bool, false)                                                            # If enabled, the automation start and stop VM module will generate automation account, runbooks, and schedules.
      start_hour               = optional(number, 7)                                                              # Start hour in the schedule timezone (Africa/Johannesburg, UTC+2).
      stop_hour                = optional(number, 19)                                                             # Stop hour in the schedule timezone (Africa/Johannesburg, UTC+2).
      days                     = optional(list(string), ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]) # Days on which the start/stop schedules will run.
      verbose_logging_enabled  = optional(bool, false)                                                            # Enable verbose logging for the runbook job triggered by this schedule.
      progress_logging_enabled = optional(bool, true)                                                             # Enable progress logging for the runbook job triggered by this schedule.
    }), {})
